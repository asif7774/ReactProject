...
class UserInfo extends Component {
    ...
    saveProfileData() {
        ...
        if (!firstName) {
            this.showMessage("First name is mandatory");
            return;
        }
        if (!lastName) {
            this.showMessage("Last name is mandatory");
            return;
        }
        if (this.props.auth.onboarded != true && !this.state.password) {
            this.showMessage("Password is mandatory");
            return;
        }
        let {
            url,
        ...
        axios.post(`${BASE_URL}/api/v1/user/profile`, data, { withCredentials: true })
            .then(res => {
                // first name and last name may have changes reload user
    ...
    componentDidMount() {
        ...
        axios.get(`${BASE_URL}/api/v1/user/profile/data`, { withCredentials: true })
            .then(res => {
                ...
                let companySearch = Promise.resolve({
                    data: {
                        hits: {
                            hits: []
                        }
                ...
                if (res && res.data && res.data.Item && res.data.Item.company_updated === -1) {
                    companySearch = axios.get(`${BASE_URL}/api/v1/user/search/company/${res.data.Item.company}`, { withCredentials: true });
                }
            ...
            .then(res => {
                ...
                newState = {
                    ...
                    companySuggestion: companySearch.data.hits.hits,
                ...
                <Grid.Row className="background-white extra-padding-sides">
                    ...
                    <Grid.Column width={13} className="mT-10">
                        <Form stackable={'true'} className="custom-input-form field-brack-md" loading={this.state.loading}>
                            ...
                            <Form.Group widths={2}>
                                <Form.Field className="validation-input required">
                                    <Form.Input onChange={this.handleEmailChange.bind(this)} value={this.state.primaryEmail} label='EMAIL 1' />
                                    {/* verified popup*/}
                                    {
                    ...
                    <Grid.Column width={16}>
                        ...
                        <Button
                            ...
                            onClick={this.saveProfileData.bind(this)}
                            floated='right'
                            size='mini'
                            color='blue'
                        >
                            {this.props.auth ? 'NEXT' : 'REGISTER'}
                        </Button>
